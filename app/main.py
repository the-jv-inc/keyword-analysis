...
def logout():
    ...
    for key in ['credentials', 'authenticated', 'sites']:
    ...
    if st.session_state.keyword_data is None:
        ...
        with tab2:
            if st.session_state.authenticated:
                ...
                st.info("Google Cloud Consoleで取得した認証情報（JSON）を貼り付けてください")
                creds_json = st.text_area("認証情報JSON", height=100, label_visibility="collapsed")
                if creds_json and save_credentials_json(creds_json):
